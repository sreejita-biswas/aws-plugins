...
func getFlags() {
	...
	flag.StringVar(&availabilityZone, "available_zone", "us-west-1a", "available zone")
	flag.Float64Var(&cpuCritical, "cpu_critical_over", 80, "Trigger a critical if cpu usage is over a percentage")
	flag.Float64Var(&cpuWarning, "cpu_warning_over", 40, "Trigger a warning if cpu usage is over a percentage")
	flag.Float64Var(&memoryCritical, "memory_critical_over", 80, "Trigger a critical if memory usage is over a Bytes")
	flag.Float64Var(&memoryWarning, "memory_warning_over", 40, "Trigger a warning if memory usage is over a Bytes")
	flag.Float64Var(&diskCritical, "disk_critical_over", 80, "Trigger a critical if disk usage is over a Bytes")
	flag.Float64Var(&diskWarning, "disk_warning_over", 40, "Trigger a warning if disk usage is over a Bytes")
	flag.Float64Var(&conectionCritical, "connections_critical_over", 80, "Trigger a critical if connection number is over a number")
